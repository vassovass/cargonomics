...
| D29 | 2026-04-17 | Apr 17 session | **"Your Instructors" feels lazy.** Vasso: want a second, forward-thinking variant that echoes the hero's ship + world + tech integration. Keep current `.trainer-card-v2` as deprecated option in styleguide. New variant lands on `index.html` and `course.html`. Best-practice research on trainer showcases required before build. | High. Currently reads as a generic bio card; weakens "industry veterans" promise that sits under "Learn from Industry Veterans". | Research (B2B ed + logistics + executive showcase patterns), draft a `trainer-card-v3` / `trainers-v3` variant with editorial portrait + data-chips (years, regions, carriers, cohorts) + subtle shipping-network motif; ship with both variants in `styleguide.html` (v2 marked deprecated, v3 as default); swap live pages to v3 once approved. Share screenshots before cutover. | Medium-Large | Vasso | open (awaiting direction lock) |
