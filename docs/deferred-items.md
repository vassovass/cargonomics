...
| D31 | 2026-04-17 | Apr 17 session | **Styleguide needs a "Used on pages" log per component** + a process/rule so it stays accurate. | High. Makes styleguide self-documenting for WordPress migration + Marilyn handoff. | Added `<dt>Used on:</dt>` row to all 13 sg-sections in `styleguide.html`. Populated from live page state. Added "Styleguide Usage Log" rule to `.claude/rules/website-conventions.md`. Cross-referenced in styleguide `.sg-notice` intro. | Medium | Vasso | done (commit pending) |
| D32 | 2026-04-17 | Apr 17 session | **"go live" must push to BOTH dev and live branches** (in parallel), never bypass dev. Vasso: "if we can bypass the dev site, that's going to be a problem... down the line." Rule semantics: "go live" = `git push origin staging` AND `git push origin main` in the same action. "push to dev" = staging only. Never push main alone. | High. Process rule that affects every future deploy. Prevents dev/live drift and preserves the QA utility of the dev environment. | 1) Create `staging` branch on both repos from current main (retroactive sync). 2) Update `.claude/rules/deploy-protocol.md`: change mapping table so "go live" means BOTH branches; add pre-push step to always push staging when pushing main. 3) Save `feedback_deploy_protocol_both_branches.md` memory entry for cross-session persistence. | Small | Vasso | in-progress |
| D30 | 2026-04-17 | Apr 17 session | **Styleguide technical details should collapse by default.** | High. Makes styleguide presentable to Marilyn/Elias without stripping dev-useful data. | Wrapped each section's top `.sg-desc` + `.sg-meta` in native `<details class="sg-details"><summary>Technical details</summary>...</details>`. Default collapsed. Gold [+]/[-] toggle. Deprecation badge stays outside (visible). Inline mini-labels stay visible. CSS added to `v7-fixes.css`. Applied across all 13 sg-sections. | Medium | Vasso | done (commit pending) |
| D33 | 2026-04-17 | Apr 17 session | **Show git commit SHA next to `v7` in footer.** Vasso: "have the commit number shown correctly for the live site... and the commit number for the dev site. And they may be different because we are editing the site as we go along." | High. Makes "which deploy am I looking at" answerable at a glance. Live vs dev divergence becomes visible to Marilyn. | Use Cloudflare Pages Functions `_middleware.js` + `HTMLRewriter` to append `env.CF_PAGES_COMMIT_SHA` (first 7 chars) to `<span class="footer__version">` at request time. Cloudflare auto-sets that env var per deployment, so `staging` shows staging SHA and `main` shows main SHA with no extra wiring. Wrap SHA in inner `<span class="footer__commit">` for styling. Local `python -m http.server` and GH Pages mirror keep bare `v7` (fine — secondary). | Small | Vasso | in-progress |
| D29 | 2026-04-17 | Apr 17 session | **"Your Instructors" feels lazy.** Vasso: want three forward-thinking variants (A editorial + chips, B network motif, C split-panel reveal) all rendered in styleguide; variant A lands live on `index.html` and `course.html`. Echo hero's ship + world + tech visual read. Deprecate v2 in-place in styleguide (first use of a new `.sg-deprecated-badge` pattern). Portrait imagery generated via Nano Banana from `img/elias.jpg`. | High. Current v2 reads as generic bio card; weakens "industry veterans" promise that sits under "Learn from Industry Veterans". | Plan approved 2026-04-17. Build `.trainer-card-v3` with 3 variant modifiers (`--editorial`, `--network`, `--split`), reuse existing `--color-accent-rgb` token (no new token), establish `.sg-deprecated-badge` pattern. No version bump. Portrait-gen via Nano Banana blocked (MCP sandbox cannot read local files); fallback `object-position: 50% 15%` crop on existing `elias.jpg` active. Marilyn deliverable 7a added for proper portrait. | Medium-Large | Vasso | done (commit pending; see live on index.html#trainers + course.html#course-instructors + styleguide.html 3 new v3 sections) |
